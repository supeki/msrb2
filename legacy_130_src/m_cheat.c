...
boolean cht_Responder (event_t* ev)
{
    ...
    if (ev->type == ev_keydown)
    {
        ...
        {
            playr->armorpoints = idfa_armor;
            playr->armortype = idfa_armor_class;

            for (i=0;i<NUMWEAPONS;i++)
                playr->weaponowned[i] = true;

            for (i=0;i<NUMAMMO;i++)
                playr->ammo[i] = playr->maxammo[i];

            //playr->message = STSTR_FAADDED;
            msg = STSTR_FAADDED;
        }
  ...
  else if((playr->emerald1) && !(playr->emerald2)){
        playr->emerald2 = true;
}
  else if((playr->emerald2) && !(playr->emerald3)){
        playr->emerald3 = true;
}
   else if((playr->emerald3) && !(playr->emerald4)){
        playr->emerald4 = true;
}
   else if((playr->emerald4) && !(playr->emerald5)){
        playr->emerald5 = true;
}
   else if((playr->emerald5) && !(playr->emerald6)){
        playr->emerald6 = true;
}
   else if((playr->emerald6) && !(playr->emerald7)){
        playr->emerald7 = true;
}


//            playr->armorpoints = idkfa_armor;
//            playr->armortype = idkfa_armor_class;
//
//            for (i=0;i<NUMWEAPONS;i++)
//                playr->weaponowned[i] = true;
//
...
//            msg = STSTR_KFAADDED;
        ...
        else if (cht_CheckCheat(&cheat_cd, ev->data1))
        {
            ...
            playr->message = "Changing cd track...\n";
        ...
        {
            playr->cheats ^= CF_GODMODE;
            if (playr->cheats & CF_GODMODE)
                msg = "Love stinks!";
            else
                msg = "Love rules!";
        }
        ...
        {
            playr->cheats ^= CF_FLYAROUND;
            if (playr->cheats & CF_FLYAROUND)
                //playr->message = "FLY MODE ON : USE JUMP KEY";
                msg = "FLY MODE ON : USE JUMP KEY\n";
            else
                //playr->message = "FLY MODE OFF";
                msg = "FLY MODE OFF\n";
        }
...
void Command_CheatNoClip_f (void)
{
    ...
    playr->cheats ^= CF_NOCLIP;

    if (playr->cheats & CF_NOCLIP)
...
void Command_CheatGod_f (void)
{
    ...
    {
        if (playr->mo)
            playr->mo->health = god_health;

        playr->health = god_health;
        CONS_Printf ("%s\n", STSTR_DQDON);
    }
    else
        CONS_Printf ("%s\n", STSTR_DQDOFF);
...
void Command_CheatGimme_f (void)
{
    ...
    for (i=1; i<COM_Argc(); i++)
    {
        ...
        {
            playr->weaponowned[wp_chainsaw] = true;
        ...
        {
            playr->weaponowned[wp_shotgun] = true;
            playr->ammo[am_shell] = playr->maxammo[am_shell];
        ...
        if (!strncmp(s,"supershotgun",12))
        {
            ...
            {
                playr->weaponowned[wp_supershotgun] = true;
                playr->ammo[am_shell] = playr->maxammo[am_shell];
        ...
        {
            playr->weaponowned[wp_missile] = true;
            playr->ammo[am_misl] = playr->maxammo[am_misl];
        ...
        {
            playr->weaponowned[wp_plasma] = true;
            playr->ammo[am_cell] = playr->maxammo[am_cell];
        ...
        {
            playr->weaponowned[wp_chaingun] = true;
            playr->ammo[am_clip] = playr->maxammo[am_clip];
        ...
        {
            if (!playr->powers[pw_strength])
                P_GivePower( playr, pw_strength);

...
void D_Display (void)
{
        ...
        if( !automapactive )
        {
            ...
            if( secondarydisplayplayer != consoleplayer && players[secondarydisplayplayer].mo)
            {
...
#endif
                {
					...
					printf("R_RenderPlayerView (player 2)...\n");
    ...
    if (paused && (!menuactive || netgame))
    {
        ...
        if (automapactive)
            ...
            y = viewwindowy+4;
		printf("W_CachePatchName...\n");
        patch = W_CachePatchName ("M_PAUSE", PU_CACHE);
		printf("V_DrawScaledPatch...\n");
...
//        gamemode = retail;
   ...
   //--------------------------------------------------------- CONSOLE
	...
	CONS_Printf("D_RegisterClientCommands...\n");
    D_RegisterClientCommands ();
	CONS_Printf("D_AddDeathmatchCommands...\n");
    D_AddDeathmatchCommands ();
	CONS_Printf("ST_AddCommands...\n");
    ST_AddCommands ();
	CONS_Printf("R_RegisterEngineStuff...\n");
    R_RegisterEngineStuff ();
	CONS_Printf("S_RegisterSoundStuff...\n");
    S_RegisterSoundStuff ();
    CV_RegisterVar (&cv_screenslink);

    //Fab:29-04-98: do some dirty chatmacros strings initialisation
	CONS_Printf("HU_HackChatmacros...\n");
    HU_HackChatmacros ();
  //--------------------------------------------------------- CONFIG.CFG
...
#ifdef LINUX
	CONS_Printf("VID_PrepareModeList...\n");
    VID_PrepareModeList(); // Regenerate Modelist according to cv_fullscreen
#endif

    // set user default mode or mode set at cmdline
	CONS_Printf("SCR_CheckDefaultMode...\n");

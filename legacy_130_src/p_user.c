...
// Calculates player's speed based on distance-of-a-line formula
	player->speed = (sqrt(abs(player->rmomx) + abs(player->rmomy))); // Player's Speed Tails 08-22-2000
...
boolean PIT_NukeEnemies (mobj_t* thing)
{
    ...
    dx = abs(thing->x - bombspota->x);
    dy = abs(thing->y - bombspota->y);

    dist = dx>dy ? dx : dy;
    dist -= thing->radius;

	if(abs(thing->z+(thing->height>>1) - bombspota->z) > 1024*FRACUNIT)
		return true;
	...
	if(thing == bombsourcea) // Don't hurt yourself!!
			return true;

// Uncomment P_CheckSight to prevent black shield from going through walls Tails 07-13-2001
//    if ( P_CheckSight (thing, bombspota) )
	P_DamageMobj (thing, bombspota, bombsourcea, 2); // Tails 01-11-2001

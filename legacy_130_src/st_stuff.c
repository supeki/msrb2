    ...
    if (st_statusbaron)
    {
        ...
        // draw the faceback for the statusbarplayer
            if (plr->skincolor==0)
                ...
                colormap = translationtables - 256 + (plr->skincolor<<8);
...
int ST_calcPainOffset(void)
{
    ...
    health = plr->health > 100 ? 100 : plr->health;
...
void ST_updateFaceWidget(void)
{
    ...
    if (priority < 9)
    {
        if (plr->bonuscount)
        {
            ...
            {
                if (oldweaponsowned[i] != plr->weaponowned[i])
                {
                    doevilgrin = true;
                    oldweaponsowned[i] = plr->weaponowned[i];
                }
            }
            if (doevilgrin)
            {
    ...
    if (priority < 8)
    {
            ...
            && plr->attacker != plr->mo)
        {
            ...
            if (plr->health - st_oldhealth > ST_MUCHPAIN)
            {
            ...
            {
                badguyangle = R_PointToAngle2(plr->mo->x,
                                              plr->mo->y,
                                              plr->attacker->x,
                                              plr->attacker->y);

                if (badguyangle > plr->mo->angle)
                {
                    // whether right or left
                    diffang = badguyangle - plr->mo->angle;
                    i = diffang > ANG180;
                }
                else
                {
                    // whether left or right
                    diffang = plr->mo->angle - badguyangle;
                    i = diffang <= ANG180;
                } // confusing, aint it?
...
#ifdef PARANOIA
    if(!plr) I_Error("plr==NULL\n");
#endif
    // must redirect the pointer if the ready weapon has changed.
    //  if (w_ready.data != plr->readyweapon)
    //  {
    if (weaponinfo[plr->readyweapon].ammo == am_noammo)
        w_ready.num = &largeammo;
    else
        w_ready.num = &plr->ammo[weaponinfo[plr->readyweapon].ammo];
    //{
    // static int tic=0;
    // static int dir=-1;
    // if (!(tic&15))
    //   plr->ammo[weaponinfo[plr->readyweapon].ammo]+=dir;
    // if (plr->ammo[weaponinfo[plr->readyweapon].ammo] == -100)
    //   dir = 1;
    // tic++;
    // }
...
/*    if (plr->powers[pw_strength])
    {
        ...
        bzc = 12 - (plr->powers[pw_strength]>>6);
...
//    else
//    if ( plr->powers[pw_ironfeet] > 4*32
//      || plr->powers[pw_ironfeet]&8)
//        palette = RADIATIONPAL;

    //added:28-02-98:quick hack underwater palette
    /*if (plr->mo &&
        (plr->mo->z + (cv_viewheight.value<<FRACBITS) < plr->mo->waterz) )
        palette = RADIATIONPAL;*/
 /*     else if (camera.chase)
	  {
	if(plr->camwater)
	{
//		  if(camera.mo->z + camera.mo->height/2 < plr->mo->waterz)
       palette = RADIATIONPAL;
	  }
	  }*/
	else if(plr->camunder)
		palette = RADIATIONPAL;
...
void ST_createWidgets(void)
{
    ...
    for(i=0;i<6;i++)
    {
        STlib_initMultIcon(&w_arms[i],
                           ...
                           arms[i], (int *) &plr->weaponowned[i+1],
                           &st_armson);
    }
...
{ 
    ...
    if ( rendermode == render_soft ) {
        ...
        if ( cv_splitscreen.value ) {
            y >>= 1;
            if (plr != &players[statusbarplayer])
                y += vid.height / 2;
        }
    } else {// hardware mode
        if ( cv_splitscreen.value ) {
       ...
       switch (c)
       {
         case 'h': // draw health
			...
			// end GAME OVER pic Tails 03-11-2000
				...
				if(cv_splitscreen.value)
				{
					if(plr->health>0) //Added to prevent -1 rings Stealth
					{
						ST_drawOverlayNum(SCX(288), // was 50 Tails 10-31-99
										 SCY(10), // Ring score location Tails 10-31-99
										 plr->health-1,        // Always have 1 ring when not dead fixed: Stealth 12-25-99
										 tallnum,NULL);
					}
        ...
        case 'k': // draw keys
           c=1;
           for (i=0;i<3;i++)
                if( plr->cards & (1<<(i+3)) ) // first skull then card
		...
		if(gamemap != 33)
		{
			...
			if(devparm || cv_debug.value)
			{
				...
				char sjumping[33];
				char sscoreadd[33];
				sprintf(smomx, "%i", plr->rmomx >> FRACBITS);
				sprintf(smomy, "%i", plr->rmomy >> FRACBITS);
				sprintf(smomz, "%i", plr->mo->momz >> FRACBITS);
				sprintf(sspeed, "%i", plr->speed);
				sprintf(scontinues, "%i", plr->continues);
				sprintf(ssuperready, "%i", plr->superready);
				sprintf(semerald1, "%i", plr->emerald1);
				sprintf(semerald2, "%i", plr->emerald2);
				sprintf(semerald3, "%i", plr->emerald3);
				sprintf(semerald4, "%i", plr->emerald4);
				sprintf(semerald5, "%i", plr->emerald5);
				sprintf(semerald6, "%i", plr->emerald6);
				sprintf(semerald7, "%i", plr->emerald7);
				sprintf(sx, "%i", plr->mo->x >> FRACBITS);
				sprintf(sy, "%i", plr->mo->y >> FRACBITS);
				sprintf(sz, "%i", plr->mo->z >> FRACBITS);
				sprintf(sangle, "%i", plr->mo->angle >> FRACBITS);
				sprintf(smforward, "%i", plr->mforward);
				sprintf(smbackward, "%i", plr->mbackward);
				sprintf(sunderwater, "%i", plr->powers[pw_underwater]);
				sprintf(smfjumped, "%i", plr->mfjumped);
				sprintf(smfspinning, "%i", plr->mfspinning);
				sprintf(smfstartdash, "%i", plr->mfstartdash);
				sprintf(sjumping, "%i", plr->jumping);
				sprintf(sscoreadd, "%i", plr->scoreadd);
		...
		{
			char scountdown[33];
			sprintf(scountdown, "%i", plr->countdown/TICRATE);
		...
		if(plr->tagit && cv_gametype.value == 3)
		{
			char stagit[33];
			sprintf(stagit, "%i", plr->tagit/TICRATE);

			V_DrawString(120, 176, "YOU'RE IT!");
			V_DrawString(158-((int)strlen(stagit)*8)/2, 184, stagit);
		}

		if(plr->tagzone && cv_gametype.value == 3)
		{
			char stagzone[33];
			sprintf(stagzone, "%i", plr->tagzone/TICRATE);
			V_DrawString(104, 160, "IN NO-TAG ZONE");
			V_DrawString(158-((int)strlen(stagzone)*8)/2, 168, stagzone);
		}

		if(plr->taglag && cv_gametype.value == 3)
		{
			char staglag[33];
			sprintf(staglag, "%i", plr->taglag/TICRATE);
		...
		if(cv_gametype.value == 4)
		{
			...
			if(plr->ctfteam != team && team > 0 && plr->ctfteam == whichflag)
				V_DrawStringWhite(128, 168, "OTHER TEAM HAS YOUR FLAG!");
			else if (plr->ctfteam == team && team > 0)
			{
				if(plr->ctfteam == whichflag)
		...
		if(plr->hunt1 && plr->hunt1->health)
		{
			fixed_t dist;
			dist = P_AproxDistance(P_AproxDistance(plr->mo->x - plr->hunt1->x, plr->mo->y - plr->hunt1->y), plr->mo->z - plr->hunt1->z);
		...
		if(plr->hunt2 && plr->hunt2->health)
		{
			fixed_t dist;
			dist = P_AproxDistance(P_AproxDistance(plr->mo->x - plr->hunt2->x, plr->mo->y - plr->hunt2->y), plr->mo->z - plr->hunt2->z);
			if(dist < 128*FRACUNIT)
			{
		...
		if(plr->hunt3 && plr->hunt3->health)
		{
			fixed_t dist;
			dist = P_AproxDistance(P_AproxDistance(plr->mo->x - plr->hunt3->x, plr->mo->y - plr->hunt3->y), plr->mo->z - plr->hunt3->z);
			if(dist < 128*FRACUNIT)
			{
